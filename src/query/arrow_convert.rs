...
pub fn metrics_to_batch(store: &Store) -> RecordBatch {
    ...
    RecordBatch::try_new(schema, columns).expect("schema mismatch in metrics_to_batch")
}
...
fn append_metric_row(
    timestamp: &mut UInt64Builder,
    metric_name: &mut StringBuilder,
    metric_type: &mut StringBuilder,
    value: &mut Float64Builder,
    count: &mut UInt64Builder,
    sum: &mut Float64Builder,
    service_name: &mut StringBuilder,
    attributes: &mut MapBuilder<StringBuilder, StringBuilder>,
    resource: &mut MapBuilder<StringBuilder, StringBuilder>,
    ts: u64,
    name: &str,
    mtype: &str,
    val: Option<f64>,
    ...
    match cnt {
        Some(c) => count.append_value(c),
        None => count.append_null(),
    }
    match sm {
    ...
    service_name.append_value(svc);
    append_kv_map(attributes, dp_attrs);
    append_kv_map(resource, res_attrs);
}
